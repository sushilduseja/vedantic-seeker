...
interface VishnuData {
  names: VishnuName[];
  categories: Record<string, number[]>;
  searchIndex: Record<string, number[]>;
}

const cachedVishnuData: { data: VishnuData | null } = { data: null };

async function loadVishnuData(): Promise<VishnuData> {
  if (cachedVishnuData.data) return cachedVishnuData.data;
  const response = await fetch('/data/vishnu-sahasranama.json');
  if (!response.ok) throw new Error('Failed to load Vishnu data');
  cachedVishnuData.data = await response.json();
  return cachedVishnuData.data as VishnuData;
}

export async function findRelevantName(query: string, _lang: 'en' | 'hi' = 'en'): Promise<VishnuName[]> {
  if (!query || query.trim().length === 0) return [];

  try {
    const data = await loadVishnuData();
    ...
    Object.entries(data.searchIndex).forEach(([term, ids]) => {
      if (term.toLowerCase().includes(lowerQuery)) {
        ids.forEach((id) => matchingIds.add(id));
      }
    });

    // Search in name attributes and text
    data.names.forEach((name) => {
      if (name.attributes.some((attr) => attr.toLowerCase().includes(lowerQuery))) {
        matchingIds.add(name.id);
      }
      if (
        name.sanskrit.includes(query) ||
        name.transliteration.toLowerCase().includes(lowerQuery) ||
        name.translation.toLowerCase().includes(lowerQuery) ||
    ...
    return data.names.filter((name) => matchingIds.has(name.id));
  } catch (error) {
